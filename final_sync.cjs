...
function cleanString(str) {
  let newStr = str;
  // Revert "Smart Learning" to "Brain Games Hub"
  newStr = newStr.replace(/Smart\s+Learning/gi, 'Brain Games Hub');
  // Hindi
...
function deepClean(obj) {
  for (let key in obj) {
    if (typeof obj[key] === 'string') {
      ...
      if (
        key === 'junior' &&
        (obj[key].toLowerCase() === 'kids' || kidsWords.some((w) => obj[key].includes(w)))
      ) {
        obj[key] = 'Junior';
      }
...
function processLocale(locale) {
  ...
  if (json.home && json.home.categories) {
    ...
    if (
      json.home.categories.junior &&
      (json.home.categories.junior.toLowerCase().includes('kids') ||
        kidsWords.some((w) => json.home.categories.junior.includes(w)))
    ) {
